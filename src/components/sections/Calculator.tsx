...
const initialTimeSlots: TimeSlot[] = [{
  ...
  time: "11:00-12:00",
  label: "11:00",
  description: "Предобеденное время",
  coverage: 6,
  selected: false
}, {
  id: "12",
...
const Calculator = () => {
  ...
  let pricePerSec = 1.5;
  if (stationsCount >= 6) pricePerSec = 1.1;else if (stationsCount >= 5) pricePerSec = 1.2;else if (stationsCount >= 3) pricePerSec = 1.3;
  ...
  const toggleStation = (id: string) => {
    setStations(stations.map(s => s.id === id ? {
      ...s,
      selected: !s.selected
    } : s));
  };
  ...
  const toggleSlot = (id: string) => {
    setTimeSlots(timeSlots.map(s => s.id === id ? {
      ...s,
      selected: !s.selected
    } : s));
  };
  ...
  return <section className="flex-1 p-4 md:p-6 overflow-y-auto animate-fade-in">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="glass-card p-4 md:p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {stations.map(station => <button key={station.id} onClick={() => toggleStation(station.id)} className={cn("relative p-3 rounded-xl border-2 transition-all duration-300 text-left group overflow-hidden", station.selected ? "border-primary bg-gradient-to-br from-primary/10 to-primary/5" : "border-border hover:border-primary/40 bg-card")}>
                    ...
                    {station.selected && <div className="absolute top-1.5 right-1.5 w-4 h-4 rounded-full bg-primary flex items-center justify-center">
            ...
            <div className="glass-card p-4">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-1.5">
                {timeSlots.map(slot => <button key={slot.id} onClick={() => toggleSlot(slot.id)} className={cn("relative p-2 rounded-lg border transition-all duration-300 text-center group", slot.selected ? "border-primary bg-primary/10 shadow-sm" : "border-border hover:border-primary/40 bg-card")}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-1 mt-2">
                ...
                {isMaxCoverage && <p className="text-[10px] text-green-600 font-medium">
          ...
          <div className="lg:col-span-2">
            <div className="glass-card p-4 md:p-6 sticky top-4 space-y-4 md:space-y-6 border-2 border-primary/20 bg-gradient-to-br from-card to-primary/5">
              ...
              <div className="space-y-3 pt-4 border-t border-border">
                ...
                <Input placeholder="Ваше имя *" value={formName} onChange={e => setFormName(e.target.value)} className="bg-background/80 border-border" />
                <Input placeholder="Телефон (необязательно)" value={formPhone} onChange={e => setFormPhone(e.target.value)} className="bg-background/80 border-border" />
                <Input placeholder="Email *" type="email" value={formEmail} onChange={e => setFormEmail(e.target.value)} className="bg-background/80 border-border" />
                <Button className="w-full gap-2 h-12 text-base" onClick={handleSubmit} disabled={isSubmitting}>
