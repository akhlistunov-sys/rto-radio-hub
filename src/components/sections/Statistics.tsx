...
const stationData = [{
  ...
  reach: "3 200 – 4 000",
  age: "35–65 лет",
  segment: "Взрослая лояльная аудитория",
  logo: logoRetro
}, {
  name: "Авторадио",
  reach: "2 900 – 3 600",
  age: "25–55 лет",
  segment: "Автомобилисты с доходом",
  logo: logoAutoradio
}, {
  name: "Радио Дача",
  reach: "2 900 – 3 600",
  age: "35–60 лет",
  segment: "Семейная аудитория",
  logo: logoDacha
}, {
  name: "Радио Шансон",
  reach: "2 600 – 3 200",
  age: "30–60+ лет",
  segment: "Мужская аудитория 30+",
  logo: logoShanson
}, {
  name: "Юмор FM",
  reach: "1 800 – 2 400",
  age: "25–45 лет",
  segment: "Активные люди 25+",
  logo: logoHumor
}, {
  name: "Love Radio",
  reach: "600 – 800",
  age: "18–35 лет",
  segment: "Молодежь и трендсеттеры",
  logo: logoLove
}];
const keyStats = [{
  value: "66 000+",
...
const efficiencyStats = [{
  value: "~11 391",
  label: "Уникальный суточный охват (6 станций)"
}, {
  value: "~341 730",
  label: "Контактов за месяц"
}, {
  value: "0,17 ₽",
  label: "Стоимость контакта (CPC)"
}];
const sources = ["Mediascope — замеры аудитории российского радиорынка, 2025", "ВЦИОМ — социология радиопотребления в регионах", "RADIOPORTAL.RU — аналитика регионального вещания", "Собственные расчеты на основе численности населения (66,000 суммарно), возрастной структуры и зоны покрытия передатчиков (35 км радиус)"];
const Statistics = () => {
  return <section className="flex-1 p-4 md:p-6 overflow-y-auto animate-fade-in">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {keyStats.map((stat, index) => <div key={stat.label} className="glass-card p-4 text-center animate-slide-up" style={{
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-2 px-2 text-muted-foreground font-medium hidden sm:table-cell">Слушатели, в сутки </th>
              ...
              <tbody>
                {stationData.map((station, index) => <tr key={station.name} className="border-b border-border/50 hover:bg-secondary/30 transition-colors animate-fade-in" style={{
                animationDelay: `${index * 50}ms`
              }}>
                    <td className="py-3 px-2">
                      <div className="flex items-center gap-2">
                        <img src={station.logo} alt={station.name} className="w-8 h-8 object-contain rounded-md bg-white" />
        ...
        <div className="glass-card p-4 md:p-6 bg-gradient-to-br from-primary/5 to-transparent">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {efficiencyStats.map((stat, index) => <div key={stat.label} className="p-4 rounded-xl bg-secondary/50 text-center">
        ...
        <div className="glass-card p-4 md:p-6">
          ...
          <ul className="space-y-2">
            {sources.map((source, index) => <li key={index} className="text-sm text-muted-foreground flex items-start gap-2">
