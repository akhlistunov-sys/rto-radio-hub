...
const ModernIndex = () => {
  ...
  return <div className="min-h-screen bg-background">
      {/* Floating Header */}
      <motion.header className="fixed top-0 left-0 right-0 z-50 px-4 py-4" initial={{
      y: -100
    }} animate={{
      y: 0
    }} transition={{
      duration: 0.5
        ...
        <div className="max-w-7xl mx-auto">
          <div className="glass-card rounded-2xl px-4 py-3 flex items-center justify-between backdrop-blur-xl border border-border/50">
            {/* Logo */}
            <motion.img src={logoRTO} alt="РТО" className="h-10 object-contain cursor-pointer" onClick={() => setActiveTab("hero")} whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }} />
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navItems.map(item => <button key={item.id} onClick={() => setActiveTab(item.id)} className={cn("px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300", activeTab === item.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-secondary")}>
                  {item.label}
                </button>)}
            </nav>
            ...
            {/* Mobile Menu Button */}
            <button className="md:hidden p-2 rounded-xl hover:bg-secondary" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          ...
          {/* Mobile Menu */}
          <AnimatePresence>
            {mobileMenuOpen && <motion.div className="md:hidden absolute left-4 right-4 top-20 glass-card rounded-2xl p-4 border border-border/50" initial={{
            opacity: 0,
            y: -20
          }} animate={{
            opacity: 1,
            ...
            y: -20
              ...
              }} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all", activeTab === item.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary")}>
